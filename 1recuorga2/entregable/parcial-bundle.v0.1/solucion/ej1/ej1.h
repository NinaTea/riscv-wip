...
typedef struct {
	uint32_t tipo; // 0
	uint32_t usos; // 4
	uint64_t unused0; // Reservamos espacio 8
	uint64_t unused1; // Reservamos espacio 16
} texto_cualquiera_t; // 24
 ...
 */
typedef struct {
	uint32_t tipo; //0 + (usa 4 bytes)
	uint32_t usos; //4 + (usa 4 bytes)
	uint64_t tamanio; // 8 
	const char* contenido; // 16
} texto_literal_t; //24 
...
typedef struct {
	uint32_t tipo; // 0
	uint32_t usos; // 4
	texto_cualquiera_t* izquierda; //8 
	texto_cualquiera_t* derecha; //16
} texto_concatenacion_t; //24
